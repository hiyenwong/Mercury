...
public class SnowFlowerUtils {
    ...
    public static class SnowFlowerUtilsInstance {
        SnowFlowerUtilsInstance () {
            throw new IllegalStateException("Unity class");
        }
        private static final SnowFlowerUtils INSTANCE = new SnowFlowerUtils();
    }

    public static SnowFlowerUtils getInstance() {
        return SnowFlowerUtilsInstance.INSTANCE;
    }

    private SnowFlowerUtils(long dataCenterId) {
    ...
    private SnowFlowerUtils() {
        long machineId = generateMachineId();
        long dataCenterId = RandomUtils.nextLong(1, 10);
        if (dataCenterId > MAX_DATACENTER_NUM || dataCenterId < 0) {
            throw new IllegalArgumentException("dataCenterId can't be greater than MAX_DATACENTER_NUM or less than 0");
        }
        if (machineId > MAX_MACHINE_NUM || machineId < 0) {
